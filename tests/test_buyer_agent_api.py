...
def test_x402_buy_blocking_uses_extended_timeout_for_post_payment_delivery(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Seller proof verification may take longer after CAW payment lands."""
    ...
    class FakeResponse:
        def read(self) -> bytes:
            ...
            return json.dumps({"content": "delivered", "proof": {"ok": True}}).encode()
    ...
    class FakeUrlLibRequestModule:
        ...
        @staticmethod
        def urlopen(req, timeout=0):
            urlopen_calls.append({"url": req.url, "timeout": timeout})
    ...
    class FakeCawClient:
        ...
        def wait_for_transaction_complete(self, tx_id, timeout=0):
            return {"transaction_hash": "0xfeedface"}
    ...
    result = app_module._api_x402_buy_blocking(
        app_module.X402BuyRequest(
            service_id=202,
            query="Find market analysis for an AI commerce launch",
            wallet_address=BUYER_WALLET,
    ...
    assert urlopen_calls[0]["timeout"] == 15
    assert "tx_hash=0xfeedface" in urlopen_calls[1]["url"]
    assert app_module.POST_PAYMENT_X402_DELIVERY_TIMEOUT_SECONDS == 180
    assert urlopen_calls[1]["timeout"] == app_module.POST_PAYMENT_X402_DELIVERY_TIMEOUT_SECONDS
    assert urlopen_calls[1]["timeout"] != 30

def test_x402_buy_blocking_timeout_after_payment_mentions_tx_hash(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """If seller delivery times out after payment, show recovery context."""
    ...
    assert "x402 buy failed after CAW step" in exc_info.value.detail
    assert "The read operation timed out" in exc_info.value.detail
    assert "Payment tx_hash: 0xfeedface" in exc_info.value.detail

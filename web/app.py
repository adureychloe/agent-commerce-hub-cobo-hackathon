...
POST_PAYMENT_X402_DELIVERY_TIMEOUT_SECONDS = 180
...
def _api_x402_buy_blocking(payload: X402BuyRequest) -> dict[str, Any]:
    ...
    try:
        ...
        resp = urllib.request.urlopen(retry_req, timeout=POST_PAYMENT_X402_DELIVERY_TIMEOUT_SECONDS)
        result = json_safe(_json.loads(resp.read()))
        ...
        recovery_context = f" Payment tx_hash: {tx_hash}." if tx_hash else ""
        raise HTTPException(
            status_code=500,
            detail=f"x402 buy failed after CAW step: {exc}.{recovery_context}",
        ) from exc
